...
class TestSimuCell(TestCase):
    def test_should_move_1_meter_right(self):
        ...
        blob = SimuCell(world=World("tests/test_world.yml"), initLocation=np.array([0.0, 0.0]), initHeading=0.0)
        blob.speed = 1.0
        blob.update()
    ...
    def test_should_move_1_meter_down(self):
        ...
        blob = SimuCell(world=World("tests/test_world.yml"), initLocation=np.array([0.0, 0.0]), initHeading=90.0)
        blob.speed = 1.0
        blob.update()
        np.testing.assert_almost_equal(blob.location, expectedDestination, decimal=1)
    ...
    def test_should_move_1_meter_diagonally_SE(self):
        ...
        blob = SimuCell(world=World("tests/test_world.yml"), initLocation=np.array([0.0, 0.0]), initHeading=45.0)
        blob.speed = 1.0
        blob.update()
        np.testing.assert_almost_equal(blob.location, expectedDestination, decimal=1)
